...
private object ClassPath {
    ...
    private[util] def readLines(source: Source): Seq[String] = {
        try {
            ...
            case ex: MalformedInputException => Seq.empty
        } finally {
            source.close()
        }
    }
...
object LoadService {
    ...
    def apply[T: ClassManifest](): Seq[T] = synchronized {
        ...
        (classNames ++ classNamesFromResources).distinct flatMap { className =>
            ...
            try {
                ...
                case NonFatal(ex) =>
                    DefaultLogger.log(
                        Level.DEBUG,
                        s"LoadService: failed to instantiate '$className' for the request "
                            + s"service '$ifaceName'",
                        ex)
                    None
            }
